...
function VideoBlockView({ data }: { data: Record<string, unknown> }) {
  ...
  if (!url) return <div className="bg-ink-50 dark:bg-[#102a1d] rounded-xl p-6 text-center text-ink-400 dark:text-[#6b7394]">Відео не завантажено</div>;
  return embedUrl ? (
...
function FileBlockView({ data }: { data: Record<string, unknown> }) {
  ...
  return (
    <div className="flex items-center gap-3 bg-ink-50 dark:bg-[#102a1d] border border-ink-200 dark:border-[#1f4d36] rounded-xl p-4">
      <div className="w-11 h-11 rounded-xl bg-white dark:bg-[#163a28] border border-ink-200 dark:border-[#1f4d36] flex items-center justify-center text-2xl flex-shrink-0">📎</div>
