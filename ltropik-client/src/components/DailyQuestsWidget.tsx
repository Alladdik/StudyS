...
export function DailyQuestsWidget() {
  ...
  if (loading) return <div className="h-24 animate-pulse bg-ink-50 rounded-2xl" />;
    ...
    <div className={cx(
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <p className="font-bold text-ink-900 text-sm">Щоденні завдання</p>
        </div>
        <span className={cx(
          'text-xs font-bold px-2 py-1 rounded-full',
          allDone ? 'bg-emerald-100 text-emerald-700' : 'bg-ink-100 text-ink-600'
        )}>
          {done}/{total}
        </span>
      </div>
      ...
      {/* Progress bar */}
      <div className="h-1.5 bg-ink-100 rounded-full mb-3 overflow-hidden">
        <div
      ...
      <div className="flex flex-col gap-1.5">
        {quests.map(q => (
          <div key={q.id} className={cx(
            ...
            q.isCompleted ? 'opacity-60' : 'bg-ink-50'
          )}>
            <span className="text-base flex-shrink-0">{q.quest.icon}</span>
            <div className="flex-1 min-w-0">
              <p className={cx('font-semibold text-ink-800 truncate', q.isCompleted && 'line-through text-ink-400')}>
                {q.quest.title}
              </p>
            </div>
