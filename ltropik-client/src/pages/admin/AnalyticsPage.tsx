...
export function AnalyticsPage() {
  ...
  return (
    <Layout title="Аналітика" subtitle="Дані платформи в реальному часі">
      ...
      {/* ── Tabs ──────────────────────────────────────────────────────────── */}
      <div className="flex gap-1 mb-5 bg-ink-100 rounded-2xl p-1 w-fit flex-wrap">
        {([
          ['overview', '📊 Активність'],
          ['courses',  '📚 Курси'],
          ['revenue',  '💰 Дохід'],
          ...
          <button key={t} onClick={() => setTab(t)}
            className={cx('px-4 py-2 rounded-xl text-sm font-semibold transition',
              tab === t ? 'bg-white text-brand-700 shadow-sm' : 'text-ink-500 hover:text-ink-800')}>
            {label}
          </button>
        ))}
      </div>
            ...
            <Card className="p-5">
              ...
              <div className="flex flex-col gap-1.5">
                {churn.slice(0, 5).map(r => (
                  <div key={r.id} className="flex items-center justify-between py-1.5 px-3 rounded-xl hover:bg-ink-50 transition">
                    <div>
              ...
              <Card className="overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {courses.map((c, i) => (
                        <tr key={c.id} className="border-b border-ink-50 last:border-0 hover:bg-ink-50/60 transition">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              ...
              <Card key={s.label} className="p-5 flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-ink-50 flex items-center justify-center text-2xl flex-shrink-0">{s.icon}</div>
