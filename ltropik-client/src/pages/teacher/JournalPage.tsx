          ...
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr className="border-b border-ink-100 dark:border-[#1c3a2a]">
                <th className="text-left px-5 py-3.5 text-xs font-bold text-ink-400 dark:text-[#6b7394] uppercase tracking-wider sticky left-0 bg-white dark:bg-[#0e2218] z-10">Студент</th>
                {dates.map((date) => (
            ...
            <tbody>
              {students.map((stud) => {
                ...
                return (
                  <tr key={sid} className="border-b border-ink-50 dark:border-[#102a1d] last:border-0 hover:bg-ink-50/60 dark:hover:bg-[#102a1d]/60 transition">
                    <td className="px-5 py-3 font-semibold text-ink-800 dark:text-[#e8eaf0] sticky left-0 bg-white dark:bg-[#0e2218] z-10 border-r border-ink-50 dark:border-[#102a1d]">{stud.name}</td>
                    {dates.map((date) => {
                          ...
                          {isToday ? (
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <select
                                ...
                                className={cx('bg-white dark:bg-[#102a1d] dark:text-[#e8eaf0] border rounded-lg px-2 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-brand-200 transition',
                                  selectedLesson ? 'border-ink-200 dark:border-[#1f4d36] cursor-pointer' : 'border-ink-100 dark:border-[#1c3a2a] opacity-50 cursor-not-allowed')}
                              ...
                              {gradeOptions.length > 0 && (
                                <select
                                  ...
                                  disabled={!selectedLesson}
                                  className={cx('bg-white dark:bg-[#102a1d] border rounded-lg px-2 py-1.5 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-brand-200 transition text-brand-700 dark:text-brand-400',
                                    selectedLesson ? 'border-brand-200 cursor-pointer' : 'border-ink-100 dark:border-[#1c3a2a] opacity-50 cursor-not-allowed')}
                                  title={!selectedLesson ? 'Спочатку оберіть урок вище' : ''}>
                                  <option value="">— Оцінка —</option>
                                  {gradeOptions.map((v) => (
